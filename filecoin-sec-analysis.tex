...
\newcommand{\papertitle}{A security analysis of the Filecoin protocol against rational adversaries}
...
\newcommand{\dom}{\ensuremath{H^\star}\xspace}
...
\newcommand{\sigpolyevalc}{\ensuremath{\mathsf{\bar{s}_{\sigma3}}}\xspace}
...
\newcommand{\ZeroH}{\ensuremath{Z_{H^{\star}}} \xspace}
...
\newcommand{\idpermpoly}{\ensuremath{\mathsf{z_1}}\xspace}
\newcommand{\sigpermpoly}{\ensuremath{\mathsf{z_2}}\xspace}
...
\newcommand{\maxcost}{\ensuremath{C}\xspace}
\newcommand{\cost}{\ensuremath{\mathsf{c}}\xspace}
\newcommand{\exprev}{\ensuremath{E}\xspace}
\newcommand{\sealcost}{\ensuremath{\mathsf{S}}\xspace}
\newcommand{\powtable}{\ensuremath{\mathsf{T}}\xspace}
\newcommand{\secind}{\ensuremath{\mathsf{i}}\xspace}
\newcommand{\tick}{\ensuremath{\mathsf{t}}\xspace}
...
Ideally, we would have wanted to have a proof of space time against all polynomial time adversaries; i.e. 
being able to have short proofs that an efficient \adv has certain files in memory continuously over a period of time. 
%In \filecoin we also want 
However this seems to require tools that are not ready today, at least in production, like verifiable delay functions, zk-SNARKs handling tens of billions of gates, and better recursive zk-SNARKs.
...
\begin{dfn}\label{dfn:rationalsec}
  A protocol \prot is secure up to cost \maxcost if the strategy of cost $\cost \leq \maxcost $ that maximizes expected revenue \exprev is the honest strartegy.
 \end{dfn}
 ...
 \begin{enumerate}
  \item  The cost \sealcost of sealing a sector (assuming for simplicity they all have the same size).
 \end{enumerate}

 
Important objects:
\begin{enumerate}
 \item The power table \powtable.
 \item The sequence of tickets.
...
\end{enumerate}
 ...
 \por{\tick}{\secind}  - checks retrievability of sector \secind with randomness from 
 ticket \tick.
 \section{biasing randomness}
